...
var arrayUtil = {};
arrayUtil.map = function (arr, callback) {
    var i = 0, l = arr && arr.length || 0, out = new Array(l);
...
crypto.ttc = new function () {
    var POW2 = 4;//Math.pow(2,2);
    var POW3 = 8;//Math.pow(2,3);
    var POW4 = 16;//Math.pow(2,4);
    var POW8 = 256;//Math.pow(2,8);
    var POW16 = 65536;//Math.pow(2,16);
    var POW24 = 16777216;//Math.pow(2,24);
    var iv = null;
    var boxes = {
        p: [
    ...
    function init(key) {
        var k = arrayUtil.map(key.split(""), function (item) {
    ...
    this.getIV = function (type) {
        var out = type || crypto.types.String;
        switch (out) {
            case crypto.types.Hex: {
                return arrayUtil.map(iv, function (item) {
                    return (item <= 0xf ? '0' : '') + item.toString(16);
                }).join("");			//	string
            }
            case crypto.types.Raw: {
                return iv;				//	array
            }
            default: {
    ...
    this.setIV = function (data, inputType) {
        var ip = inputType || crypto.types.String;
        var ba = null;
        switch (ip) {
            case crypto.types.Hex: {
                ba = [];
    ...
    this.encrypt = function (plaintext, key, ao) {
        var out = crypto.types.String;
        var mode = crypto.modes.ECB;
        if (ao) {
            if (ao.type) out = ao.type;
        ...
        var cipher = [], count = plaintext.length >> 3, pos = 0, o = {}, isCBC = (mode == crypto.modes.CBC);
    ...
    this.setIV("0000000000000000", crypto.types.Hex);
}();
